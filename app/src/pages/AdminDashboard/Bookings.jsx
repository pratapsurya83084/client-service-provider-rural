...
const FILTERS = ['All', 'Pending', 'Confirmed', 'InProgress', 'Completed', 'Cancelled']
...
const Bookings = () => {
  ...
  return (
    ...
    >
      ...
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-[26px] font-extrabold text-[#e6edf3] tracking-tight">All Bookings</h1>
        <p className="text-[13px] text-[#4ade80] mt-1 font-medium">
          {filtered.length} booking{filtered.length !== 1 ? 's' : ''} in database
        </p>
      </div>
          ...
          {FILTERS.map(f => {
            ...
            return (
              <button
                ...
                style={active && st
                  ? { color: st.color, borderColor: st.border, background: 'transparent' }
                  : active
                  ? { color: '#4ade80', borderColor: '#4ade80', background: 'transparent' }
          ...
          <table className="w-full min-w-[900px] border-collapse">
            <thead>
              <tr className="border-b border-[#21262d]">
                {['ID', 'SERVICE', 'CUSTOMER', 'MOBILE', 'PROVIDER', 'DATE', 'STATUS', 'ADDRESS', 'CREATED'].map(h => (
            ...
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={9} className="text-center text-[#484f58] py-10 text-[13px]">
                    No bookings found.
                  </td>
                </tr>
              ) : filtered.map((b, i) => {
                const st = STATUS_STYLES[b.status] || { color: '#8b949e', border: '#8b949e' }
                return (
                  <tr
                    key={b.id}
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-[13px] text-[#c9d1d9]">{b.customer}</td>
                    <td className="px-4 py-4 text-[12.5px] text-[#8b949e]"
                      style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {b.mobile}
                    </td>
                    <td className="px-4 py-4 text-[13px] text-[#c9d1d9]">{b.provider}</td>
                    <td className="px-4 py-4 text-[12.5px] text-[#8b949e]"
                      style={{ fontFamily: "'JetBrains Mono', monospace" }}>
                      {b.date}
                    </td>
                    <td className="px-4 py-4">
                      <span
                        ...
                        style={{ color: st.color, borderColor: st.border, background: 'transparent' }}
                      >
                        {b.status}
                      </span>
                    </td>
